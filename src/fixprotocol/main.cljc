(ns fixprotocol.main
  "Kotodama WASM entrypoint for the Fixprotocol clean-room actor (L4) — Clojure port.
  ...
  facts (`fixprotocol.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "fixprotocol")
(def tier "L4")
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "fixproto_ins"
   ...
   {:entity "Execution"  :plural "executions"  :id-prefix "fixproto_exe"
    :fields [:orderId :quantity :price :executedAt] :required [:quantity :price]
    :coerce {:quantity :float :price :float}   :refs {:orderId "Order"}}
   {:entity "MarketData" :plural "marketdatas" :id-prefix "fixproto_mar"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "fixproto_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"fixprotocol.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "fixprotocol-compat" :tier tier :entities entities} 200])
...
(defn start! [] :fixprotocol-compat/ready)
